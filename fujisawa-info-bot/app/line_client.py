"""LINE Messaging API クライアントと Webhook 署名検証 (Phase 1)。

`line-bot-sdk` v3 を薄くラップする。 Phase 1 では Reply Message のみ使用。
Push Message は Phase 6 (Emergency push) で追加する。

driving-license-bot/app/services/line_client.py と同じ思想で、 bytes 直の
署名検証パスを残してテスト容易性を上げる。
"""
...
class LineBotClient:
    """LINE Messaging API の v3 SDK 薄ラッパ。"""
    ...
    def verify_signature(self, body: bytes, signature: str) -> None:
        """LINE 公式仕様の HMAC-SHA256 + base64 で X-Line-Signature を検証する。

        SDK の `WebhookParser` も内部で同じ検証を行うが、 bytes 直の検証パスを
        残すことで「parse せず認証だけ確認」 が必要な場面 (Pub/Sub publish 前等)
        にも使い回せる。
        """
        if not signature:
    ...
    def parse_events(self, body: bytes, signature: str) -> list[Event]:
        """署名検証 + LINE event リストへのパース。 失敗時 InvalidSignatureError。"""
        try:
    ...
    def reply_text(self, reply_token: str, messages: list[str]) -> None:
        """複数行のテキスト Reply Message。 LINE 制限で 1 回最大 5 通。"""
        if not messages:
            return
