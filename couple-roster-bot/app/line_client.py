...
`line-bot-sdk` v3 を薄くラップする。本システムでは以下を使う:
- Reply Message（テキスト返信）
- Message Content 取得（CSV ファイル / 画像のバイト列ダウンロード）

`fujisawa-info-bot` / `driving-license-bot` と同じく、bytes 直の署名検証パスを
残してテスト容易性を上げる。
"""
...
class LineBotClient:
    """LINE Messaging API v3 SDK の薄ラッパ。"""
    ...
    def verify_signature(self, body: bytes, signature: str) -> None:
        """HMAC-SHA256 + base64 で X-Line-Signature を検証する。"""
        if not signature:
    ...
    def reply_text(self, reply_token: str, messages: list[str]) -> None:
        """複数行のテキスト Reply。LINE 制限で 1 回最大 5 通。"""
        if not messages:
            return
    ...
    def get_message_content(self, message_id: str) -> bytes:
        """ファイル / 画像メッセージの実体（バイト列）をダウンロードする。"""
        with ApiClient(self._configuration) as api_client:
            blob = MessagingApiBlob(api_client)
            content = blob.get_message_content(message_id)
