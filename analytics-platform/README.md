...
> - **Cloud Workflows + Cloud Scheduler** (`workflows/dbt_pipeline.yaml` + `scripts/deploy_orchestration.sh`、§3.5)
> - **Terraform IaC** (GCS / BQ / Artifact Registry / IAM。`terraform/` 配下、§3.3.1)
> - **Cloud Monitoring アラート** (Workflow FAILED / dbt Job failed / dbt Job slow + email channel、§3.5.2)
>
> 未着手: Langfuse on GKE / VPC + GKE / Step 10 各エージェントの env 切替。方針は設計書 §3 / §4 / §9 / §13 に準拠。
...
### 3.3.1 Terraform IaC (実装済 / Step 1-3)

`terraform/` 配下で **GCS バケット / BigQuery dataset / Artifact Registry / Service Accounts + IAM** を Terraform 管理。Cloud Run Job / Workflows / Scheduler は CI/CD で頻繁に更新するため shell script のままにし、静的なデータ基盤と IAM だけを TF で固める方針。

#### 含まれるリソース

| カテゴリ | 中身 | ファイル |
|---|---|---|
| GCS | `analytics-raw` / `analytics-payloads` / `analytics-dead-letter` の 3 bucket + lifecycle (NEARLINE 30d → COLDLINE 90d → ARCHIVE 365d、dead_letter は 90d で削除) | `terraform/gcs.tf` |
| Artifact Registry | dbt image 用 Docker repo | `terraform/artifact_registry.tf` |
| BigQuery | `analytics_raw` / `analytics_staging` / `analytics_marts` dataset + GCS Hive external table | `terraform/bigquery.tf` |
| IAM | SA × 4 (`sa-uploader` / `sa-dbt` / `sa-workflow` / `sa-scheduler`) + bucket / dataset / project レベルの role binding | `terraform/iam.tf` |
...
gcloud services enable storage.googleapis.com artifactregistry.googleapis.com \
  bigquery.googleapis.com run.googleapis.com workflows.googleapis.com \
  cloudscheduler.googleapis.com iam.googleapis.com iamcredentials.googleapis.com

# 3. tfvars / backend.tf を準備
...
```
Cloud Scheduler (cron, default: 0 * * * *)
    │ POST → Workflows execution API
    ▼
Cloud Workflows: analytics-platform-dbt-pipeline
    │ workflows/dbt_pipeline.yaml
    │ - googleapis.run.v2.projects.locations.jobs.run で Cloud Run Job 起動
    │ - transient API error は Workflow 側で 3 回 retry (指数バックオフ)
    │ - 失敗時: Slack webhook 通知 (任意) → execution は FAILED で終了
    ▼
Cloud Run Jobs (analytics-platform-dbt)
    - args=[run-and-test] で Cloud Run Job entrypoint を起動 (Step 7 と同じ image)
    - dbt-bigquery + Workload Identity で BigQuery に認証
    - GCS の JSONL は external table 経由で参照されるため Load Job は不要
```

**実装ファイル**:
- `workflows/dbt_pipeline.yaml` — Workflow 定義 (main + notify_slack subworkflow)
- `scripts/deploy_orchestration.sh` — Workflow + Scheduler を idempotent にデプロイ
- `Makefile` — `deploy-orchestration` / `trigger-workflow` ターゲット

**デプロイ手順** (初回 / Step 8 反映):

```bash
# 1. SA 作成 (一度だけ)
gcloud iam service-accounts create sa-workflow --project=${ANALYTICS_GCP_PROJECT}
gcloud iam service-accounts create sa-scheduler --project=${ANALYTICS_GCP_PROJECT}

# 2. SA に必要な権限を付与
PROJECT="${ANALYTICS_GCP_PROJECT}"
# Workflow が Cloud Run Job を起動できるように
gcloud projects add-iam-policy-binding $PROJECT \
  --member=serviceAccount:sa-workflow@$PROJECT.iam.gserviceaccount.com \
  --role=roles/run.invoker
gcloud projects add-iam-policy-binding $PROJECT \
  --member=serviceAccount:sa-workflow@$PROJECT.iam.gserviceaccount.com \
  --role=roles/logging.logWriter
# Scheduler が Workflow を起動できるように
gcloud projects add-iam-policy-binding $PROJECT \
  --member=serviceAccount:sa-scheduler@$PROJECT.iam.gserviceaccount.com \
  --role=roles/workflows.invoker

# 3. API 有効化 (一度だけ)
gcloud services enable workflows.googleapis.com cloudscheduler.googleapis.com run.googleapis.com \
  --project=${ANALYTICS_GCP_PROJECT}

# 4. Workflow + Scheduler をデプロイ (idempotent)
...
**運用**:
- **手動実行**: `gcloud workflows execute analytics-platform-dbt-pipeline --data='{"project":"...", ...}'`
- **ログ確認**: Cloud Logging で `resource.type=workflows.googleapis.com/Workflow`
- **失敗検知**: Workflow execution が FAILED になると Cloud Monitoring の組み込みメトリクス (`workflowexecutions.googleapis.com/finished_execution_count`) が増えるため、Phase 5.9 で alerting を設定する
- **頻度変更**: `ANALYTICS_SCHEDULE_CRON` を変更して `make deploy-orchestration` 再実行
- **Slack 通知**: `ANALYTICS_SLACK_WEBHOOK_URL` を設定して再デプロイすると、Workflow が失敗時に Slack に投稿する

**日次ジョブ** (将来の追加):
- `dbt run --select marts` フル rebuild (Phase 6+ で incremental 化)
- Enrichment (content_summary / content_keywords 生成)
- Cloud Scheduler を 2 個目作って `--schedule="0 4 * * *"` で別 Workflow を呼ぶ

### 3.5.2 Cloud Monitoring アラート (実装済 / Step 9)

`terraform/monitoring.tf` で **3 つの alert policy + email notification channel** を Terraform 管理。

#### Alert ラインナップ

| policy | 検知 | metric | しきい値 | 重要度 |
|---|---|---|---|---|
| `workflow_failed` | Cloud Workflows execution が FAILED | `workflowexecutions.googleapis.com/finished_execution_count` (label `status=FAILED`) | 5 分間に 1 件以上 | high |
| `dbt_job_failed` | Cloud Run Job が failed で完了 | `run.googleapis.com/job/completed_execution_count` (label `result=failed`) | 5 分間に 1 件以上 | high |
| `dbt_job_slow` | Cloud Run Job duration がしきい値超過 | `run.googleapis.com/job/execution/duration_ms` (MAX) | `dbt_job_max_duration_seconds` (default 1800 秒) | medium |

すべての policy は `auto_close = 30 分〜1 時間` で healthy 復帰時に自動クローズ。`user_labels.step = "9"` をつけているため、Cloud Monitoring 上で本 PR 由来のものだけ絞り込み可能。
...
enable_alerts      = true                    # false でアラートを丸ごと作らない (sandbox 用)
dbt_job_max_duration_seconds = 1800          # 0 にすれば slow alert だけスキップ
```

`make tf-plan && make tf-apply` で反映。`notification_email = ""` のままでも policy は作られるが通知先がないため、Cloud Monitoring コンソールで手動確認するモードになる。

#### Slack 通知について

Cloud Monitoring 標準の Slack 通知は OAuth 連携が必要なため、本 IaC では email のみ。Slack に飛ばしたいパスは 2 通り:

1. **Step 8 の Workflow 内 webhook** (`ANALYTICS_SLACK_WEBHOOK_URL` env): Workflow 失敗時に Slack に投稿。
2. **Pub/Sub 経由**: 通知 channel に `pubsub` を選び、Pub/Sub → Cloud Run で Slack に転送する関数を別途立てる。Phase 5.x で必要なら追加。
...
| 手順 | 内容 | 備考 |
|---|---|---|
| 1 | GCP プロジェクト + Artifact Registry (+ 将来: VPC / GKE) を準備 | ✅ Artifact Registry は Terraform 管理済 (§3.3.1)。VPC / GKE は未着手 |
| 2 | Langfuse on GKE を Helm chart でデプロイ (Cloud SQL / Memorystore / GCS を紐付け) | 設計書 §4.2 (未着手) |
| 3 | GCS バケット 3 種 + ライフサイクルルールを作成 | ✅ Terraform 管理済 (§3.3.1) |
| 4 | BigQuery データセット + external table を作成 | ✅ Terraform 管理済 (§3.3.1)。`scripts/gcp_bootstrap.sh` も併存 (gcloud 派の fallback) |
...
| 8 | Cloud Scheduler + Cloud Workflows を設定 | ✅ 実装済 (`workflows/dbt_pipeline.yaml` + `scripts/deploy_orchestration.sh`、§3.5 参照) |
| 9 | Cloud Monitoring のアラートポリシー作成 | ✅ 実装済 (`terraform/monitoring.tf`、§3.5.2)。Workflow FAILED / dbt Job failed / dbt Job slow + email channel |
...
| **Phase 5 (進行中)** | GCP 環境 — GCS Uploader / Payload + dbt-bigquery + Cloud Run Job + Workflows/Scheduler + Terraform IaC + Monitoring | 🔶 大部分実装 (Step 1 部分 / Step 3-9 完了、Step 2 Langfuse / Step 10 デプロイ未着手) | §3.0 / §3.0.1 / §3.3.1 / §3.4 / §3.5 / §3.5.1 / §3.5.2 |
