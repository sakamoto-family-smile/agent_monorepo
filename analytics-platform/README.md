...
> **ステータス (2026-04)**: Phase 5 の一部 (**L2-f GCSPayloadWriter / L4 GCSTransport / env 駆動 factory**) を先行実装済。`ANALYTICS_STORAGE_BACKEND=gcs` で GCS 出力に切替可能。
> BigQuery 連携 (L5 `dbt-bigquery`) / オーケストレーション / Langfuse on GKE は未着手。
> 方針は設計書 §3 / §4 / §9 / §13 に準拠。
