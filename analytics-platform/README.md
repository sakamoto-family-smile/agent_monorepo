...
>
> 未着手: Cloud Run Jobs コンテナ化 / Cloud Workflows + Scheduler / Cloud Monitoring / Langfuse on GKE / Terraform。方針は設計書 §3 / §4 / §9 / §13 に準拠。
...
| 7 | Cloud Run Jobs のコンテナ image を build & push (dbt / uploader 2 本) | `Dockerfile` 新規 (未着手) |
...
| **Phase 5 (進行中)** | GCP 環境 — GCS Uploader / Payload + dbt-bigquery (cross-DB マクロ + external table source) | 🔶 部分実装 (Uploader / Payload / dbt-bigquery 完了。Cloud Run Jobs / Workflows / Langfuse on GKE 未着手) | §3.0 / §3.0.1 / §3.4 |
