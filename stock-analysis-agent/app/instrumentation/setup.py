...
設計:
  ...
  - `shutdown_observability()` で
    ...
    の順で cleanup。
  - `ANALYTICS_ENABLED=false` の場合は NoOpSink を使い、JSONL は書かれない (テスト用)。
"""
...
async def start_upload_loop() -> None:
    ...
    `analytics_upload_interval_seconds <= 0` の場合は何もしない (shutdown 時の最後の
    1 回のみで運用)。Cloud Run のような長期稼働プロセスを前提に呼ぶ。
    """
    global _upload_task
    s = _settings()
...
async def shutdown_observability() -> None:
    ...
    # 3. 最終 upload (gcs backend なら GCS まで送る)
    await _run_uploader_once()
