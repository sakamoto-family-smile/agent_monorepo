...
| 2026-06-05 | Done (P1) | **本番移行を実行完了**。`shared-pg`(db-g1-small) に 3DB 集約 (question_bank / fujisawa_kb_db / piyolog)。検証 OK (piyolog_events 490 / fujisawa pages 6740 + pgvector 健全 / 全 Cloud Run Ready)。旧 `driving-license-bot-pg` は削除、旧 `piyolog` は stop 保持 (ロールバック用)。shared-pg は deletion_protection=true。残: 1〜2 週検証後に旧 piyolog 削除、API レベル削除保護のコード化、driving-license-bot tfvars の tier 既定整理 |
