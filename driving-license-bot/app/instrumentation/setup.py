"""analytics-platform の初期化と DI コンテナ。

piyolog-analytics の同名ファイルを driving-license-bot 向けに調整。
- env `ANALYTICS_DISABLED=true` で NoOpSink にフォールバック
- env `OTEL_EXPORTER_OTLP_ENDPOINT` 未設定時は OTLP exporter を立てない
"""
...
class NoOpSink:
    """analytics 無効時に AnalyticsLogger に渡すダミー sink。"""
...
def setup_observability() -> None:
    """`main.py` の lifespan から呼ぶ。複数回呼んでも no-op。"""
    ...
    if settings.analytics_enabled:
        raw_dir = Path(settings.analytics_data_dir) / "raw"
        payloads_dir = Path(settings.analytics_data_dir) / "payloads"
        ...
        _content_router = ContentRouter(
            ...
            inline_threshold_bytes=settings.analytics_content_inline_threshold_bytes,
        )
        logger.info(
...
async def shutdown_observability() -> None:
    """`main.py` の lifespan 終了で呼ぶ。バッファに残ったイベントをフラッシュ。"""
    ...
    except Exception:  # noqa: BLE001 — フラッシュ失敗で起動を止めない
