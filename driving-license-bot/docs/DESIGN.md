...
### 3.1.5 Phase 2-F 実装：MCP Proxy 経由化準備 + e-Gov 接続クライアント

設計書 §15.2 / [INTEGRATIONS.md MCP Proxy 経由化](./INTEGRATIONS.md#mcp-proxy-経由化) と整合。
Phase 4 で law-mcp 等の実 MCP を立てる前段として、外部 HTTP の egress を
proxy 経由に統一する受け皿と、最初の実 HTTP 統合（e-Gov）を整備した。

```
app/integrations/
├── proxied_http_client.py    httpx.AsyncClient ファクトリ。
│                              SECURITY_HTTP_PROXY_URL env で proxy 経由切替
└── egov_law_client.py        e-Gov 法令検索 API v2 の薄いクライアント
                              （fetch_law_text + health_check）

scripts/verify_mcp_proxy.sh        proxy 到達性 + .mcp.json declarations 表示
scripts/verify_egov_connectivity.py  e-Gov API スモークチェック CLI
```

**現状の役割**:
- `.mcp.json` は宣言（Phase 4 で実 MCP サーバを立てるまでトラフィックは流れない）
- `proxied_http_client` は今すぐ使える（e-Gov / 警察庁等の任意 HTTP egress）
- `EgovLawClient` は Phase 4 で law-update-pipeline と Fact Checker の live
  検証に使う本番接続点の素材
...
| Phase | mode | 主体 |
|---|---|---|
| 2-F | passive 固定 | proxy declarations のみ、トラフィック無し |
| 4 | passive で 1〜2 週運用 → 違反校正 | 実 MCP 起動 → DLP/injection 校正 |
| 5 | active に切替 | 違反時 block + alert |
