...
2. `terraform destroy`（Cloud Run / Firestore / Secret 枠 / SA / Artifact Registry repo）
...
```bash
PURGE_STATE=true GOOGLE_CLOUD_PROJECT=sakamoto-family-agent make teardown
# tfstate バケットも削除

# あるいは project ごと削除
gcloud projects delete sakamoto-family-agent
```
