...
### 推奨: `.env.secrets` + `make secrets-push` で一括投入
...
```
LINE_CHANNEL_SECRET=<貼り付け>
LINE_CHANNEL_ACCESS_TOKEN=<貼り付け>
OPERATOR_LINE_USER_IDS=Uxxx,Uyyy   # 任意
```

`scripts/push_line_secrets.sh` が:
- 各値を `printf '%s'` で stdin に流す（末尾改行混入を防ぐ）
- secret 枠が無ければ `make tf-apply` を促してエラー終了
- 投入後は byte 数を表示して目視確認可能

⚠️ teardown-app では **secret 値は消えない**ため、一度投入すれば後の cycle で
再投入不要。LINE 側で token を再発行した時だけ `.env.secrets` を更新して
`make secrets-push` を再実行する運用。
...
確認プロンプトで `yes` → app リソース（Cloud Run / Firestore / Artifact
Registry / sa-line-bot）が削除される。**Secret Manager 枠と値・WIF / tfstate /
API は残る**ため、`Terraform plan / driving-license-bot` ジョブは継続して動作し、
かつ次回の `make tf-apply` 後に secret を再投入する必要もない。
